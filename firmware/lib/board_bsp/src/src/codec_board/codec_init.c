...
static int _i2s_init(uint8_t port, esp_codec_dev_type_t dev_type, codec_init_cfg_t *init_cfg)
{
    ...
    // Bumped from the IDF default (6 desc x 240 frames = 1440 frames,
    // ~90ms at 16kHz) to 8 x 480 = 3840 frames (~240ms) -- recorder.cpp's
    // recordToSd() calls the blocking fwrite() for each 2048-byte chunk
    // directly in the same loop as the I2S read, with no producer/consumer
    // decoupling. A ~90ms DMA ring buffer leaves very little slack before
    // an SD write stall (SPI SD cards can stall well past that on a wear-
    // leveling/erase cycle) causes i2s_channel_read() to return a buffer
    // the driver already zero-filled for the overrun period -- exactly the
    // frequent short (1-10ms) zero-sample dropouts a real recording showed
    // (rec_015.wav: 1557 dropouts over 288s, median ~2.4ms, one 1.2s
    // outlier), which is what reads as "crackling." 3840 frames x 8
    // bytes/frame (32-bit stereo slots) = ~3.75KB per descriptor, within
    // the ESP32-S3 I2S DMA per-descriptor limit -- not a guess in that
    // sense, but the choice of 240ms of margin (vs some other value) is a
    // judgment call pending a real on-device re-test of the same kind of
    // recording that showed the dropouts.
