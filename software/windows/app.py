...
def index(request: Request):
    ...
    # a recording in the memo/journal sense, so they no longer render
    # inline in the main list here.
...
def jarvis_enable_live_agent(request: Request, groq_api_key: str = Form("")):
    ...
    Deepgram Voice Agent path -- see macOS app.py for the full rationale."""
    redirect = _gate(request)
    if redirect:
        return redirect
    saved = settings.get_all()
...
async def jarvis_execute_decision(request: Request):
    """Called BY THE DEVICE, not a browser -- see macOS app.py for the full
    rationale. Requires the shared secret from
    settings.get_or_create_jarvis_device_api_key() in the X-Jarvis-Key
    header, since firmware has no session cookie for _gate()."""
...
async def jarvis_search_memory(request: Request):
    """Called BY THE DEVICE -- see macOS app.py for the full rationale.
    Same X-Jarvis-Key auth as /jarvis/execute-decision."""
    expected_key = settings.get_or_create_jarvis_device_api_key()
    if request.headers.get("X-Jarvis-Key") != expected_key:
...
def jarvis_memory_facts(request: Request):
    """Called BY THE DEVICE once per live session -- see macOS app.py for
    the full rationale. Same X-Jarvis-Key auth."""
    expected_key = settings.get_or_create_jarvis_device_api_key()
    if request.headers.get("X-Jarvis-Key") != expected_key:
...
def jarvis_page(request: Request):
    ...
    they aren't a recording in the memo/journal sense, so they get the
    same top-level-page treatment /social already established for a
    different non-memo recording kind."""
    redirect = _gate(request)
    if redirect:
        return redirect
