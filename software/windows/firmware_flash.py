...
def esptool_binary_path() -> str | None:
    ...
    default_path = os.path.expanduser(os.path.join("~", ".platformio", "penv", "Scripts", "esptool.exe"))
...
def list_serial_ports() -> list:
    """COM ports a connected ESP32 board would show up as on Windows. No
    pyserial dependency added just for this -- WMI query via `wmic` (present
    on every Windows install without extra packages) is sufficient for this
    dev-only feature; falls back to an empty list (manual entry in the
    admin UI) if wmic itself isn't available."""
    try:
        proc = subprocess.run(
    ...
    for line in proc.stdout.splitlines():
        if "(COM" in line:
            start = line.rfind("(COM")
            end = line.find(")", start)
            if end != -1:
                ports.append(line[start + 1:end])
...
def flash_full(port: str, binaries_dir_override: str = None) -> dict:
    ...
    if not esptool:
        return {"ok": False, "error": "esptool not found -- install it (pip install esptool) "
                                       "or check ~/.platformio/penv/Scripts/esptool.exe"}
    if not port:
