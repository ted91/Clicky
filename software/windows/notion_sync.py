...
def _page_plain_text(page_id: str) -> str:
    ...
    participant/person relation property (see push_recording's docstring),
    only free text in the page body. One level deep only (no recursion
    into nested blocks/toggles) -- Notes/Journal pages built by this
    project are flat (headings + paragraphs), not nested."""
    parts = []
    cursor = None
    while True:
        params = {"page_size": 100}
...
def query_pages_mentioning(person_name: str, start: str = None, end: str = None) -> list:
    """New cross-page retrieval for jarvis.py's find_context -- NOT a
    rename of an existing lookup. Notes/Journal pages have no participant/
    ...
    person_name (a person, project, or topic keyword) against each
    candidate page's title + Speaker N properties + full block-content text
    (see _page_plain_text). Returns
...
def ensure_insight_properties(database_id: str):
    ...
    Notes or Journal database -- see macOS notion_sync.py for the full
    rationale. Same gate-and-PATCH pattern as ensure_publication_properties."""
...
def _insight_properties(record: dict) -> dict:
    ...
    # topic/intent phrases occasionally do.
