...
def get_or_create_jarvis_device_api_key() -> str:
    ...
    /jarvis/execute-decision route (see app.py). This is the first case of
    the device calling INTO the Mac rather than the Mac pulling from the
    device, so unlike the browser-facing routes (session-cookie gated via
    _gate()), the firmware has no session -- this key is the only thing
    standing between "anyone on the same WiFi" and "can make this app
    draft emails/add calendar events." Shown on the Jarvis settings panel
    for the user to copy into the firmware's WiFi-credentials config."""
    with _lock:
        data = _load()
